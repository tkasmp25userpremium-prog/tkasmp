...
const QuizIntro: React.FC<QuizIntroProps> = ({
  ...
  // ✅ amanin module (karena levels kamu object {1:{},2:{},3:{}})
  const module = useMemo(() => {
    return (topic as any)?.levels?.[level] || null;
  }, [topic, level]);

  // ✅ amanin array supaya .map tidak crash
  ...
  const safePurpose: string = useMemo(() => {
    const p = (topic as any)?.purpose;
    return typeof p === "string" ? p : "";
  }, [topic]);

  // ✅ fallback premium dari localStorage (premiumstore)
  const [localPremium, setLocalPremium] = useState(false);

  useEffect(() => {
    try {
      const s: any = loadPremiumState();
      setLocalPremium(!!s?.isPremium);
    } catch {
  ...
  const isFreeAllowed =
    (topic.id === "mtk-1" && level === 1) || (topic.id === "bi-1" && level === 1);
  ...
  const topicName = (topic?.name || "Materi").toString();
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 md:p-12 overflow-hidden relative">
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div>
              <div className="text-5xl mb-6">{(topic as any)?.icon || "📘"}</div>
            ...
            <div className="bg-black/40 border border-zinc-800 p-6 rounded-3xl min-w-[200px] text-center md:text-left">
              ...
              <div className="text-[10px] text-blue-400 font-black uppercase tracking-widest mt-2">
                {module?.questionCount ?? 0} Soal Tersedia
              </div>
            </div>
          </div>
            ...
            <div className="space-y-8">
              <section>
                ...
                {safeCoverage.length === 0 ? (
                  <p className="text-zinc-500 text-xs">Cakupan belum tersedia.</p>
                ) : (
